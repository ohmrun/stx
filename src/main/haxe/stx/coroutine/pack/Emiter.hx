...
@:forward abstract Emiter<O,E>(EmiterDef<O,E>) from EmiterDef<O,E> to EmiterDef<O,E>{
  ...
  @:noUsing static public function fromSignal<T>(self:stx.nano.Signal<T>){
  ...
  @:noUsing static public function fromTinkSignal<T>(self:tink.core.Signal<T>){
    ...
    return __.hold(Held.Guard(
      new Future(
        function rec(cb){
          __.log().trace('unguard');
          switch(buffer.length){
            case 0 : self.nextTime((_) -> true).map(
              (t) -> cb(__.emit(t,__.hold(Held.Guard(new Future(rec)))))
            );
            case _ : 
              __.emit(buffer.shift(),__.hold(Held.Guard(new Future(rec))));
          }
          return () -> {};
        }
      )
