...
class HClassType extends OrdCls<THClassType>{
  ...
  public function comply(a:THClassType,b:THClassType):Ordered{
    ...
    if(ord.is_not_less_than()){
      ord = new stx.assert.ord.term.ArrayOrd(Ord.Anon(op)).comply(a.interfaces,b.interfaces);
    }
    if(ord.is_not_less_than()){
      final x = new stx.assert.ord.term.ArrayOrd(Ord.Makro().Type().HClassField);
      final y = Ord.Makro().Type().HRef(x);
      ord = y.comply(a.fields,b.fields);
    }
    if(ord.is_not_less_than()){
      ord = new stx.assert.ord.term.ArrayOrd(Ord.Makro().Type().HClassField).comply(a.statics.get(),b.statics.get());
    }
    if(ord.is_not_less_than()){
      //$type(a.constructor);
    ...
    //TODO init?
    if(ord.is_not_less_than()){
      ord = new stx.assert.ord.term.ArrayOrd(Ord.Makro().Type().HClassField).comply(a.overrides.map(x -> x.get()),b.overrides.map(x -> x.get()));
    }
    return ord;
  }
  private function op(a:{t:THRef<ClassType>,params:Array<Type>},b:{t:THRef<ClassType>,params:Array<Type>}):Ordered{
    ...
    if(ord.is_not_less_than()){
      ord = new stx.assert.ord.term.ArrayOrd(Ord.Makro().Type().Type).comply(a.params,b.params);
    }
    return ord;
  }
}