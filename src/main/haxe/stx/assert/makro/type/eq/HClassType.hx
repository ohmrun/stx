...
class HClassType extends EqCls<THClassType>{
  ...
  public function comply(a:THClassType,b:THClassType):Equaled{
    ...
    if(eq.is_ok()){
      eq = new stx.assert.eq.term.ArrayEq(Eq.Anon(op)).comply(a.interfaces,b.interfaces);
    }
    if(eq.is_ok()){
      final x = new stx.assert.eq.term.ArrayEq(Eq.Makro().Type().HClassField);
      final y = Eq.Makro().Type().HRef(x);
      eq = y.comply(a.fields,b.fields);
    }
    if(eq.is_ok()){
      eq = new stx.assert.eq.term.ArrayEq(Eq.Makro().Type().HClassField).comply(a.statics.get(),b.statics.get());
    }
    if(eq.is_ok()){
      //$type(a.constructor);
    ...
    //TODO init?
    if(eq.is_ok()){
      eq = new stx.assert.eq.term.ArrayEq(Eq.Makro().Type().HClassField).comply(a.overrides.map(x -> x.get()),b.overrides.map(x -> x.get()));
    }
    return eq;
  }
  private function op(a:{t:THRef<ClassType>,params:Array<Type>},b:{t:THRef<ClassType>,params:Array<Type>}):Equaled{
    ...
    if(eq.is_ok()){
      eq = new stx.assert.eq.term.ArrayEq(Eq.Makro().Type().Type).comply(a.params,b.params);
    }
    return eq;
  }
}